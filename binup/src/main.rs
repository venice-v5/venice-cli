use std::{
    io::Read, time::{Duration}
};
use anyhow::anyhow;
use poise::serenity_prelude as serenity;
use shared::{manifest::ProgramIcon, runtime::RtBin};
use tokio::{select, time::sleep};
use vex_v5_serial::{
    commands::file::{Program, ProgramIniConfig, Project, UploadFile, USER_PROGRAM_LOAD_ADDR},
    connection::{serial, Connection},
    packets::file::{ExtensionType, FileExitAction, FileMetadata, FileVendor},
    string::FixedString,
...
async fn test(
    ctx: Context<'_>,
    #[description = "Binary file (.bin)"] binary: serenity::Attachment,
    #[description = "Program (.vpt)"] vpt: serenity::Attachment,
) -> Result<(), Error> {
